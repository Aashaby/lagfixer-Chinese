...
public class LagShieldModule extends AbstractModule implements Runnable, Listener {
    ...
    public LagShieldModule(LagFixer plugin, ModuleManager manager) {
        super(plugin, manager, AbstractModule.Impact.HIGH, "LagShield",
                new String[]{
                        // 汉化文本Monitors server load and adjusts settings during latency spikes.
                        "监控服务器负载，并在延迟飙升时动态调整设置。",
                        // 汉化文本Addresses fluctuations in server performance to mitigate delays and lag.
                        "针对服务器性能波动进行处理，减轻延迟与卡顿。",
                        // 汉化文本Dynamically adjusts settings, disables unnecessary features, and optimizes resources.
                        "动态调整配置、关闭不必要的功能，并优化资源占用。",
                        // 汉化文本Ensures smooth gameplay by minimizing the impact of performance fluctuations.
                        "通过降低性能波动带来的影响，确保游戏体验更流畅。"
                },
