...
public class MobAiReducerModule extends AbstractModule implements Listener {
    ...
    public MobAiReducerModule(LagFixer plugin, ModuleManager manager) {
        super(plugin, manager, AbstractModule.Impact.VERY_HIGH, "MobAiReducer",
                new String[]{
                        // 汉化文本Replaces creature movement to optimize and reduce behavior.
                        "替换生物移动逻辑，以优化并减少不必要的行为。",
                        // 汉化文本Addresses inefficiencies caused by default animal behavior like unnecessary random movements or constant looking around.
                        "解决默认动物行为带来的低效问题，例如无意义的随机移动或频繁转头。",
                        // 汉化文本MobAiReducer intervenes by disabling unnecessary PathFinders or replacing them with more efficient ones.
                        "MobAiReducer 会禁用不必要的寻路器（PathFinder），或将其替换为更高效的实现。",
                        // 汉化文本Crucial in scenarios with numerous animals as even minor movements can strain server resources.
                        "在生物数量很多的场景中非常关键，因为哪怕细微的移动也会给服务器资源带来压力。"
                },
