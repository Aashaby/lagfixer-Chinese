...
public class EntityLimiterModule extends AbstractModule implements Listener {
    ...
    public EntityLimiterModule(LagFixer plugin, ModuleManager manager) {
        super(plugin, manager, AbstractModule.Impact.HIGH, "EntityLimiter",
                new String[]{
                        // 汉化文本Restricts the number of entities per chunk.
                        "限制每个区块（Chunk）内的实体数量。",
                        // 汉化文本Essential for survival servers with expansive animal farms.
                        "对于拥有大型动物农场的生存服务器非常必要。",
                        // 汉化文本Prevents excessive entity accumulation and associated performance issues.
                        "防止实体过量堆积及其引发的性能问题。",
                        // 汉化文本Maintains stable performance levels even in environments with high entity density.
                        "即使在实体密度很高的环境下，也能保持性能稳定。"
