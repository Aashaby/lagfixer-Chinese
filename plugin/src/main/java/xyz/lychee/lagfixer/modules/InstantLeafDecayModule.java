...
public class InstantLeafDecayModule extends AbstractModule implements Listener {
    ...
    public InstantLeafDecayModule(LagFixer plugin, ModuleManager manager) {
        super(plugin, manager, AbstractModule.Impact.LOW, "InstantLeafDecay",
                new String[]{
                        // 汉化文本Ensures instant leaf removal, reducing leaf blocks for ticking.
                        "实现树叶即时清理，减少需要刻（tick）处理的树叶方块数量。",
                        // 汉化文本Vital for server performance by eliminating gradual leaf block processing.
                        "通过消除逐步腐烂处理带来的开销，对服务器性能至关重要。",
                        // 汉化文本Optimizes server resources for smoother gameplay without decay overhead.
                        "优化服务器资源占用，让游戏更流畅，避免腐烂逻辑的额外开销。",
                        // 汉化文本Ideal for servers with forestry, managing leaf block accumulation.
                        "适用于有大量伐木/林业活动的服务器，用于管理树叶堆积。"
