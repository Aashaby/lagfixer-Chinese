...
public class VehicleMotionReducerModule extends AbstractModule implements Listener {
    ...
    public VehicleMotionReducerModule(LagFixer plugin, ModuleManager manager) {
        super(plugin, manager, AbstractModule.Impact.LOW, "VehicleMotionReducer",
                new String[]{
                        // 汉化文本Optimizes all vehicles such as Boats and Minecarts.
                        "优化所有载具，例如船与矿车。",
                        // 汉化文本Removes minecarts with chests spawned in mineshafts.
                        "移除在废弃矿井中生成的运输矿车（带箱子的矿车）。",
                        // 汉化文本Particularly useful when minecarts are frequently used on the server.
                        "当服务器频繁使用矿车时尤其有用。",
                        // 汉化文本Enhances server performance by optimizing vehicle mechanics and removing unnecessary entities.
                        "通过优化载具机制并移除不必要的实体来提升服务器性能。"
                },
    ...
    public void onEntityPlace(EntityPlaceEvent event) {
        ...
        if (!event.isCancelled() && this.canContinue(entity.getWorld()) && entity instanceof Vehicle) {
