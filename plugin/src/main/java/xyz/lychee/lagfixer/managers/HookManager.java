    ...
    public void load() {
        for (AbstractHook hook : this.hooks.values()) {
            ...
            try {
                ...
                hook.load();
                this.addLoaded(hook);
                // 汉化文本Successfully loaded hook
                this.getPlugin().getLogger().info(" &8• &r已成功加载 Hook " + hook.getName() + "，耗时 " + t.stop().getExecutingTime() + "ms！");
            } catch (Exception ex) {
                // 汉化文本Error with enabling hook
                this.getPlugin().getLogger().info(" &8• &c启用 Hook " + hook.getName() + " 时出错，原因：" + ex.getMessage());
    ...
    public void disable() {
        for (AbstractHook hook : this.hooks.values()) {
            ...
            try {
                ...
                // 汉化文本Successfully disabled hook
                this.getPlugin().getLogger().info(" &8• &r已成功禁用 Hook " + hook.getName() + "，耗时 " + t.stop().getExecutingTime() + "ms！");
            } catch (Exception ex) {
                // 汉化文本Error with disabling hook
                this.getPlugin().getLogger().info(" &8• &c禁用 Hook " + hook.getName() + " 时出错，原因：" + ex.getMessage());
                this.getPlugin().printError(ex);
            }
        }
    }
